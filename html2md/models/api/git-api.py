...
def get_current_file_sha(repo_owner, repo_name,
                         file_path_in_repo,
                         github_token):
    api_url = f"https://api.github.com/\
    repos/{repo_owner}/{repo_name}/\
    contents/{file_path_in_repo}"
    response = requests.get(
        api_url, headers={"Authorization": f"Bearer {github_token}"}
    )
...
def upload_file_to_github(
    file_path, repo_owner, repo_name, file_path_in_repo, github_token
):
    try:
        ...
        current_sha = get_current_file_sha(
            repo_owner, repo_name, file_path_in_repo, github_token
        )
...
# repo_owner, repo_name, file_path_in_repo,
# github_token)
